...
enum ImportBatchStatus { COMMITTED UNDONE }
```
...
model Transaction {
  ...
  // dedup/batch-tracking for the CSV importer (R15)
...
Not created by the user directly — one row per CSV batch import (R15). Holds the raw uploaded files for audit and lets the import list/undo/export screens work off a single record instead of scanning `Transaction` by `importBatchId` alone.

```prisma
model ImportBatch {
  id           String            @id @default(cuid())
  userId       String
  label        String            // free text, e.g. "Nubank cartão 2023-2024"
  status       ImportBatchStatus @default(COMMITTED)
  sourceFiles  Json              // raw uploaded CSV text per file kind — audit/debug only
  rowCount     Int               // rows read from the uploaded files
  createdCount Int               // Transactions actually created (installments count individually)
  createdAt    DateTime          @default(now())
  undoneAt     DateTime?
...
`status` only ever transitions `COMMITTED → UNDONE` (undo is idempotent — a second attempt is rejected, not re-run). There is no "pending" status: the validation phase is stateless and writes nothing.
